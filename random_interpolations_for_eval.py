...
for images, labels in tqdm(celeba_loader, desc='Computing interpolations'):
    ...
    linline = lerp(*images, n=8)
    dec_geodesic = M_Dec.optimize_geodesic(*images_cuda, n_segments=8, n_iter=100).cpu()
    ebm_geodesic = optimize_ebm_geodesic(*images_cuda, log_likelihood=nested_gmm.log_prob, n_segments=8, n_subdiv=3, n_iter=10000).cpu()
    geodesic = M.optimize_geodesic(*images_cuda, n_segments=8, n_subdiv=3, n_iter=10000).cpu()

    interpolations.append(InterpolationGroup(linline, dec_geodesic, ebm_geodesic, geodesic, labels))

torch.save(interpolations, 'pickled_objects/interpolations_new.pt')
