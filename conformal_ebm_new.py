...
def ebm_curve_energy(curve: torch.Tensor, log_likelihood) -> torch.Tensor:
    dx = curve[1:] - curve[:-1]
    curve_neg_lp = -log_likelihood(curve)
    # ref = curve_neg_lp[0]
    forward = torch.sum(dx.square().sum(-1) * curve_neg_lp[:-1])
    dx = curve[:-1] - curve[1:]
    backward = torch.sum(dx.square().sum(-1) * curve_neg_lp[1:])
...
def ebm_curve_length(curve: torch.Tensor, log_likelihood) -> torch.Tensor:
    ...
    curve_neg_lp = -log_likelihood(curve)
    # ref = curve_neg_lp[0]
    forward = torch.sum(torch.sqrt(dx.square().sum(-1) * curve_neg_lp[:-1]))
    dx = curve[:-1] - curve[1:]
    backward = torch.sum(torch.sqrt(dx.square().sum(-1) * curve_neg_lp[1:]))
    return (forward + backward) / 2.0
